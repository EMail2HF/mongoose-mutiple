...
module.exports = (opt) => {
  ...
  mongoose.server = merge(defaults.server(socketOptions, reconnectOptions), opt.server);
  mongoose.replSet = merge(defaults.replSet(socketOptions, reconnectOptions), opt.replSet);
